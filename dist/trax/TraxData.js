...
export class TraxData {
    constructor(data) {
        ...
        if (lastLine.indexOf('meta') > -1) {
            const metaData = lastLine.split(';');
            for (const meta of metaData) {
                const metaAttributes = meta.split(',');
        ...
        for (let i = 0; i < channelLines.length / 2; i++) {
            if (channelLines[i * 2].length > 0) {
                const channelId = parseInt(channelLines[i * 2]);
                const channelItemSets = channelLines[(i * 2) + 1].split(';');
                const channel = new TraxChannel(channelId);
                for (const channelItemSet of channelItemSets) {
                    const channelItemData = channelItemSet.split(',');
                    if (channelItemData.length !== 2)
                        return;
